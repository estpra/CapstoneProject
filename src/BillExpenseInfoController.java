...
{
    @FXML
    private Button imageSelectButton, enterBillExpenseInfoButton;
    ...
    @FXML
    private TextField amountTxtField, dateOfEntryTxtField, billDueDateTxtField, billNameTxtField;
...
//            System.err.println(selectedFiles.size());
    ...
    //This method is activates when the user checks either true or false and sets the text for the recurring bill note and the dates note(set it up like this cuz we were having issues having the textfeild activate
    //added note for dates here for ease as it was giving us issues the original way we wanted to implement it
...
noteForDates.setText("*Note: Date of Entry and Bill Due Date entries are FINAL once bill is saved*");
    ...
    {
        try {
            //Setup the if statement in the logical order that we did becuase or(||) will only check both conditions if the first condition is false(if 1st condition is true, it will not check the 2nd) and this is the behavior we want cuz we're checking if all the required fields are empty to throw an error
            //so if one is empty, there is no need to check the others as user needs to at least fill out all required fields to be able to create a bill and if one required field is missing, there is no point in checking the other fields, and the last condition(!yesCheckBox.isSelected() & !noCheckBox.isSelected())
            //uses an &(single) cuz we want to make sure that at least one checkbox is ckecked and if one is checked, since we have code set up to return true if it is not checked, if one is checked it will return false and make the statement return false and move on to else statement; what this code essentially
            //says is return true if all required fields are empty and if its true then it goes into if statement(like usual) if one is false then keep checking the rest until all fields are false(this is achieved using or(||) operator) and the last statement returns false as single & operator will return
            //false if one of the statements is false, this wouldnt work with or as or requires both statements to be false for it to return false
            if ((amountTxtField.getText().equals("") || dateOfEntryTxtField.getText().equals("")) || (billDueDateTxtField.getText().equals("") || billNameTxtField.getText().equals("")) || (!yesCheckBox.isSelected() & !noCheckBox.isSelected()))
            {
...
//    public void expenseAddedWindow(ActionEvent event)
//    {
